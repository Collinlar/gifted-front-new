...
export default function InvoiceAssessment() {
  ...
  useEffect(()=>{
    const LoadQuizzes = async()=>{
      const response = await getAssessment(locator.state.name, locator.state.grade)
      const paidQuizzes = [response.assessment].filter(item =>
        item?.registered?.includes(id)
      );
      console.log(paidQuizzes)
      setUpdate(!update)      
      setQuizzes(()=>{return [...response.data.assessment]})
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4" style={{ color: brandColors.primary }}>
            {/* STEM Quizzes */}
          {`Assessment materials for ${locator.state.name}`}
